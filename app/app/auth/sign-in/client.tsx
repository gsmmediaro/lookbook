...
export function SignInClient({ covers }: { covers: string[] }) {
  const [state, formAction, isPending] = useActionState(signInWithEmail, null);

  // Split covers into 3 columns, shuffled for variety
      ...
      <div className="flex w-full lg:w-1/2 flex-col items-center justify-center bg-[#0D0D0D] px-6">
        <div className="w-full max-w-sm space-y-8">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-white text-balance">Welcome back</h1>
          </div>
      ...
      <div className="hidden lg:block lg:w-1/2 bg-[#141414] relative overflow-hidden">
        <div className="absolute inset-0 grid grid-cols-3 gap-3 p-6 rotate-[-6deg] scale-110">
          {columns.map((col, colIdx) => {
            ...
            // Each card: aspect 9/16 in a ~33% wide column. Approximate card height + gap.
